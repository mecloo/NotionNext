...
export default function PostHeader({ post, siteInfo, isDarkMode }) {
  ...
  return (
    <div
      id='post-bg'
    className='md:mb-0 -mb-5  relative md:flex-shrink-0 overflow-hidden w-[800rem] h-[15rem] bg-cover bg-center bg-no-repeat z-10'>
      <style jsx>{`
        .coverdiv:after {
          position: absolute;
          content: '';
      ...
      `}</style>
      <div
       className={`${isDarkMode ? 'bg-[#CA8A04]' : 'bg-[#0060e0]'} absolute w-full h-fulltop-0 py-10 flex justify-center items-center`}>
        {/* 文章背景图 */}
{/*
        <div
